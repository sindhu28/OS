...
static free_entry free_list[FREE_LIST_SIZE];
...
void manage_free_pool(page_header *temp_ptr, free_entry *free_pool, page_header *prev_ptr)
{
    ...
    {
         free_pool->free_start = temp_ptr->next_free;
...
void manage_large_page_free_pool(page_header *temp_ptr, int pages, page_header *prev_ptr, int count)
{
    ...
    if(prev_ptr == NULL)
        start_ptr = (char*)temp_ptr-(count*PAGE_SIZE);
    else
        start_ptr = prev_ptr->next_free;
    end_ptr = (char*)start_ptr+((pages-1)*PAGE_SIZE);

    if(start_ptr == free_pool->free_start)
    {
         free_pool->free_start = end_ptr->next_free;
         if(free_pool->free_start == NULL)
         {
             free_pool->free_end = NULL;
         }
...
void *malloc(size_t size) 
{
    ...
    if(size <= PAGE_SIZE/2) 
    {
        index = get_index(size);  //index of free list to allocate
        size = 1<<index;         //size to allocate to user (powers of 2)
        entry = &free_list[index];
        //check if there is any free page to allocate memory from 
        ...
        else
        {
            ...
            if(free_pool->free_start != NULL)
            {
                ...
                while(temp_ptr != NULL)
                {   
                    if((((char*)temp_ptr)+PAGE_SIZE) == (temp_ptr->next_free))
                    {
                        count++;
                    }
                    else
        ...
        if(entry->free_start && ret_addr) 
        {
            ...
            if (entry->free_start->allocated_blocks == NUMBLOCKS(size)) 
            {
                ...
                entry->free_start = temp_ptr->next_free;
                if (entry->free_start == NULL)
                    entry->free_end = NULL;
                temp_ptr->next_free = NULL;
            }
    ...
    else 
    {
        ...
        if(((size+HEADER_SIZE)%PAGE_SIZE))
            pages = ((size+HEADER_SIZE)/PAGE_SIZE) + 1;
        else
            pages = (size+HEADER_SIZE)/PAGE_SIZE;
        ...
        if(free_pool->free_start != NULL)
        {
            ...
            while(temp_ptr != NULL)
            {
                ...
                else
                {
                    ...
                    {
                        if(prev_ptr == NULL)
                            ptr = (char*)temp_ptr-(count*PAGE_SIZE);
                        else
                            ptr = prev_ptr->next_free;

                        manage_large_page_free_pool(temp_ptr, pages, prev_ptr, count);
                        //allocate this page
                        ptr->block_size = 0x8000;
                        ptr->allocated_blocks = pages;
                        ptr->next_free = NULL;
                        ret_addr = ptr+1;
                        break;
                    }
        ...
        {
            //allocate a new page using system call
            new_page = (page_header *)get_memory(pages*PAGE_SIZE);
            if(new_page)
            {
                memset(new_page, 0, sizeof(page_header));
...
void manage_free_list(size_t size, page_header *page_base)
{
    ...
    if(prev_ptr == NULL)
    {
        curr_list->free_start = ptr->next_free;
...
void split_to_pages(page_header* page_base)
{
    ...
    free_pool = &free_list[FREE_POOL];
    next_ptr = page_base->next_free;
    pages = page_base->allocated_blocks;
    temp_ptr = page_base;
    while (pages!= 1)
    {
        new_ptr = (char*)temp_ptr + PAGE_SIZE;
...
void free(void* ptr) {
	...
	RESET_BITMAP(page_base->bitmap, blockno);
        ...
        if (page_base->allocated_blocks == 1) 
        {
            ...
            //add the page in sorted order into free pool

            //remove from free list if on free list
            if(NUMBLOCKS(size)!=1)
            {
    ...
    {
        //split large pages and add to free pool
        page_base = (page_header*)ptr-1;
        return;
        temp_ptr = free_pool->free_start;
        prev_ptr = NULL;
